...
import at.techbee.spectacled.screens.note.presentation.notelist.NoteListAction
    ...
    CenterAlignedTopAppBar(
            ...
            Crossfade(multiselectItems != null) { multiselectEnabled ->
                ...
                Row {
                    if (!multiselectEnabled) {
                        TextButton(
                            onClick = {
                                onAction(NoteListAction.OnNavigateUp(true))
                            },
                        ) {
                            Row(
                        ...
                        TextButton(
                            onClick = { onAction(NoteListAction.OnClearMultiselectItems) },
                        ) {
                            Row(
                                horizontalArrangement = Arrangement.spacedBy(8.dp, Alignment.Start),
                                verticalAlignment = Alignment.CenterVertically
                        ...
                        TextButton(
                            onClick = { onAction(NoteListAction.OnSelectAllMultiselectItems) },
                        ) {
                            Text(
        ...
        actions = {
            ...
            AnimatedVisibility(multiselectItems == null) {
                ...
                Row {
                    TextButton(
                        onClick = {
                            if (isSearchBarExpanded) {
                                onAction(NoteListAction.OnSearchQueryChanged(""))
                                onAction(NoteListAction.OnCategoryFilterChanged(""))
                            }
                            onAction(NoteListAction.OnSearchBarExpanded(!isSearchBarExpanded))
                        }
                    ) {
                        Icon(
                            imageVector = if (isSearchBarExpanded) Icons.Outlined.SearchOff else Icons.Outlined.Search,
                            contentDescription = stringResource(Res.string.search_in_notes),
                            tint = onSurfaceTint
                        )
                    }
                                ...
                                DropdownMenuItem(
                                    ...
                                    onClick = {
                                        ...
                                        if (listSortedBy.name == newSortedBy.name && listSortedBy != ListSortedBy.DRAGANDDROP)
                                            onAction(NoteListAction.OnSortedByChanged(newSortedBy, !sortedAscending))
                                        else
                                            onAction(NoteListAction.OnSortedByChanged(newSortedBy, true))
                                    },
                                    trailingIcon = {
                    ...
                    TextButton(
                        onClick = {
                            onAction(
                                NoteListAction.OnViewModeChanged(
                                    when (listLayout) {
                    ...
                    if (getPlatform().platform == Platforms.DESKTOP || getPlatform().platform == Platforms.WASM) {
                        ...
                        TextButton(
                            onClick = { onAction(NoteListAction.OnToggleMultiselectItem(null)) }
                        ) {
                            Icon(
                        ...
                        TextButton(
                            onClick = { onAction(NoteListAction.OnTriggerSync) }
                        ) {
                            Icon(
            ...
            AnimatedVisibility(multiselectItems != null) {
                Row {
                    TextButton(
                        onClick = { onAction(NoteListAction.OnShowUpdateColorOfSelectedBottomSheet(true)) },
                    ...
                    TextButton(
                        onClick = { onAction(NoteListAction.OnShowDeleteSelectedItemsDialog(true)) },
                        enabled = multiselectItems?.isNotEmpty() == true
                    ) {
                        Icon(
...
@Preview
@Composable
private fun NoteSearchBar_Preview() {
...
@Preview
@Composable
private fun NoteSearchBar_blue_Preview() {
...
@Preview
@Composable
private fun NoteSearchBar_Multiselect_Preview() {
    CompositionLocalProvider(LocalContentColor provides Color.Unspecified) {
        MaterialTheme(colorScheme = getThemeForColoredSurfaces(Color.Unspecified)) {
...
@Preview
@Composable
private fun NoteSearchBar_yellow_Multiselect_Preview() {
...
@Preview
@Composable
private fun NoteSearchBar_sync_in_progress_Preview() {
    CompositionLocalProvider(LocalContentColor provides Color.Unspecified) {
        MaterialTheme {
