...
class VJournalIcsSerializer : KoinComponent {
    ...
    fun serializeVCalendar(icalEntries: List<IcalEntry>): String {
        ...
        icalEntries.forEach { icalEntry ->
            lines += serializeIcalEntry(icalEntry)
                .split("\r\n")
        }
    ...
    private fun serializeIcalEntry(icalEntry: IcalEntry): String {
        val lines = mutableListOf<String>()

        lines += "BEGIN:${spectacledVariant.syncCalendarComponent.name}"    // BEGIN:VJOURNAL or BEGIN VTODO
        ...
        lines += "END:${spectacledVariant.syncCalendarComponent.name}"    // END:VJOURNAL or END:VTODO
