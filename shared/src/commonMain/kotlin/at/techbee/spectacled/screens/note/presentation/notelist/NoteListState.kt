...
data class NoteListState(
    ...
    val displayMap: Map<ListGrouping, List<Note>>
        get() = computeDisplayMap()

    val trashbin: List<Note>
        get() = getTrashbinList()

    @OptIn(ExperimentalTime::class)
    private fun computeDisplayMap(): Map<ListGrouping, List<Note>> {

        val baseList = notes.filter { !it.syncState.isDeletedState() }
        ...
        val groupedMap: Map<ListGrouping, List<Note>> = sortedFilteredList.groupBy {
            when (listSortedBy) {
    ...
    private fun getTrashbinList()= notes.filter { it.syncState.isDeletedState() }
}